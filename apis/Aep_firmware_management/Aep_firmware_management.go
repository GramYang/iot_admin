...
//参数body: 类型json, 参数不可以为空
//  描述:body,具体参考平台api说明
func CreateFirmware(appKey string, appSecret string, productId string , firmwareName string , firmwareVersion string , body string, firmwareDesc string , createBy string ) (*http.Response, error) {
    path := "/aep_firmware_management/firmware"
    
    var headers map[string]string = nil
    var param map[string]string = make(map[string]string)
    param["productId"] = productId
    param["firmwareName"] = firmwareName
    param["firmwareVersion"] = firmwareVersion
    param["firmwareDesc"] = firmwareDesc
